...
def build_tree(grid, th: P):
    ...
    def find_children(node, grid, visited):
        row, col = node.row, node.col
        for r, c in [(row + 1, col), (row - 1, col), (row, col + 1), (row, col - 1)]:
            if (
                0 <= r < len(grid)
                and 0 <= c < len(grid[0])
                and grid[r][c] == node.value + 1
                and (r, c) not in visited
            ):
                child = Node(grid[r][c], r, c)
                visited.add((r, c))
                node.children.append(child)
                find_children(child, grid, visited)

    x, y = p_to_xy(th)
    root = Node(0, y, x)

    if root:
        visited = {(root.row, root.col)}
        find_children(root, grid, visited)
...
for th in trailheads:
    ...
    for node in end_nodes:
        if node.value == 9:
            solution += 1
        # print(f"End node at ({node.row}, {node.col}): {node.value}")
    # visualize_tree(root)
