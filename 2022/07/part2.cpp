...
int main() {
    ...
    long long needed_space = 30000000;
    ...
    long long still_needed_space = needed_space - (70000000 - calculate_dir_size("/", dir_structure));
    ...
    for (const auto& dir : dir_structure) {
        ...
        dir_sizes[dir.first] = size;
    }

    cout << "Still needed space: " << still_needed_space << endl;

    int smallest_valid_size = INT_MAX;
    string smallest_valid_dir;
    
    for (const auto& dir : dir_sizes) {
        if (dir.second >= still_needed_space && dir.second < smallest_valid_size) {
            smallest_valid_size = dir.second;
            smallest_valid_dir = dir.first;
        }
    }
    
    cout << "Directory to delete: " << smallest_valid_dir << " with size: " << smallest_valid_size << endl;
